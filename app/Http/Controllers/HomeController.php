...
class HomeController extends Controller
{
    ...
    { 
        ...
        $end_date = date("Y").'-'.date("m").'-'.'28'; //Exist bug when date is february (Generate invalid date)
        $yearly_sale_amount = [];
        ...
        $end = strtotime(date('Y-m-28'));//Exist bug when date is february (Generate invalid date) I did change to 28 but the correct will be the last day of the month

        while($start < $end)
        {
            $start_date = date("Y").'-'.date('m', $start).'-'.'01';
            $end_date = date("Y").'-'.date('m', $start).'-'.'28'; //Exist bug when date is february (Generate invalid date) I did change to 28 but the correct will be the last day of the month
